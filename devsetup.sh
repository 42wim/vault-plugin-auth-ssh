...
echo "vault write auth/ssh/login role=ubuntu cert=@tmp/sshkey-cert.pub $(createsig/createsig tmp/sshkey)"
echo ""
echo ""
echo "You can now login with publickey via:"
echo "vault write auth/ssh/login role=ubuntu2 public_key=@tmp/sshkey.pub $(createsig/createsig tmp/sshkey)"
echo ""
echo ""
